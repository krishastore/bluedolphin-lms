...
<div class="bdlms-wrap">
	...
	$learns             = isset( $course_information['what_you_learn'] ) ? $course_information['what_you_learn'] : '';
	$skills             = isset( $course_information['skills_you_gain'] ) ? $course_information['skills_you_gain'] : '';
	...
	<div class="bdlms-course-detail-nav">
		<div class="bdlms-container">
			<ul>
				<?php if ( $requirements || $learns || $skills ) : ?>
						...
						<div class="bdlms-course-requirement-box" id="course-content">
							...
							<div class="bdlms-accordion-course-content">
								<div class="bdlms-accordion">
									<?php
									$meta_key           = sprintf( \BlueDolphin\Lms\BDLMS_COURSE_STATUS, $course_id );
									$course_status      = get_user_meta( $current_user->ID, $meta_key, true );
									$course_status      = ! empty( $course_status ) ? explode( '_', $course_status ) : array();
									$current_section_id = ! empty( $course_status ) ? (int) reset( $course_status ) : 0;
									$current_item_id    = ! empty( $course_status ) ? (int) end( $course_status ) : 0;
									$inactive           = false;
									...
									foreach ( $curriculums_list as $item_key => $curriculums ) :
										...
										$section_note       = ! empty( $curriculums['section_desc'] ) ? $curriculums['section_desc'] : '';
											...
											<div class="bdlms-accordion-collapse">
												...
												<div class="bdlms-lesson-list">
													...
													foreach ( $items as $key => $item ) :
														...
														?>
														<li>
															<label <?php echo $current_curriculum && ! $course_completed ? esc_attr( 'class=in-progress' ) : ''; ?>>
															...
															<?php endif; ?>
																<span class="bdlms-lesson-class">
																	...
																	<span class="class-time-info">
																		...
																		<?php if ( $current_section_id === $item_key && $current_item_id === $item_id ) : ?>
																			...
																			<?php endif; ?>
																		<?php else : ?>
																			<?php if ( empty( $current_section_id ) && 1 === $item_key && 1 === $key ) : ?>
																				<a href="<?php echo esc_url( $course_link ); ?>" class="bdlms-btn"><?php echo esc_html_e( 'Continue', 'bluedolphin-lms' ); ?></a>
																			<?php elseif ( $inactive ) : ?>
